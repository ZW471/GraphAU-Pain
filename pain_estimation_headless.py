"""UNBC 3-class pain finetuning with BackboneOnlyPain (no AU head).

Used for SynPAIN → UNBC transfer. Identical to pain_estimation_full_generic.py
except the model is BackboneOnlyPain — backbone + spatial pool + linear pain
classifier, no AU GNN, no PE branch. Loading a SynPAIN-pretrained checkpoint
populates the backbone via strict=False; the linear classifier is randomly
initialized at fine-tune time.
"""
import os
import numpy as np
import torch
...
from model.ANFL import BackboneOnlyPain
from dataset import *
from utils import *
...
def main(conf):
    ...
    # BackboneOnlyPain ignores num_classes/neighbor_num/metric — passing them
    # for symmetry with the other model classes only.
